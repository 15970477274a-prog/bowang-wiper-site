import { TranslationDict } from "./types";

export const ru: TranslationDict = {
    navHome: "Главная",
    navProducts: "Продукция",
    navAdvantages: "Преимущества",
    navAboutUs: "О нас",
    navGetQuote: "Контакты",
    navBlog: "Блог",
    navCatalog: "Каталог (PDF)",
    heroTag: "Завод сертифицирован по ISO9001",
    heroTitle: "Профессиональный производитель щеток стеклоочистителя из Китая",
    heroSubtitle: "Ningbo Zhenhai Bowang Autoparts Co., Ltd. специализируется на OEM и ODM производстве высококачественных щеток стеклоочистителя. Мы гарантируем превосходную долговечность, идеальную очистку и глобальную экспортную логистику.",
    heroInquireBtn: "Узнать цену",
    heroProductsBtn: "Наша продукция",
    productsTitle: "Наша популярная продукция",
    productsSubtitle: "Разработаны для работы в любых погодных условиях. Изготовлены из натурального каучука высшего качества, высокоуглеродистой пружинной стали и прочного аэродинамического корпуса.",
    bestSeller: "Хит продаж",
    premiumChoice: "Премиум выбор",
    multiCompatible: "Мультисовместимые",
    universalTitle: "Универсальные щетки",
    universalDesc: "Подходят для 99% автомобилей благодаря предустановленным креплениям. Уникальный аэродинамический каркас обеспечивает равномерное прижатие и очистку без разводов даже на высоких скоростях.",
    universalSpec1: "Натуральный каучук класса AAA",
    universalSpec2: 'от 12" до 28"',
    universalSpec3: "> 1000 000 циклов",
    universalSpec4: "Предустановленный U-Hook (крючок)",
    specificTitle: "Специальные щетки (Direct Fit)",
    specificDesc: "Оригинальные сменные щетки стеклоочистителя, соответствующие люксовым и европейским моделям автомобилей (BMW, Benz, Audi, Porsche и др.) с оригинальными типами поводков.",
    specificSpec1: "Натуральный каучук с тефлоновым покрытием",
    specificSpec2: 'от 14" до 28"',
    specificSpec3: "Соответствие оригинальным спецификациям",
    specificSpec4: "Прямая установка без переходников",
    multifunctionTitle: "Мультифункциональные щетки",
    multifunctionDesc: "Оснащены сменными адаптерами. Решают проблему складских запасов, позволяя закрыть более 15 типов поводков всего несколькими моделями SKU.",
    multifunctionSpec1: "Силикон / Натуральный каучук",
    multifunctionSpec2: 'от 12" до 30"',
    multifunctionSpec3: "Более 10 адаптеров в комплекте",
    multifunctionSpec4: "Радикальное снижение затрат на склад",
    getCatalogBtn: "Получить каталог и цены",
    whyTitle: "Преимущества прямого производства",
    whySubtitle: "Прямые поставки с завода гарантируют высокое качество, конкурентоспособные цены и надежность поставок.",
    whyAdv1Title: "Цена напрямую от завода",
    whyAdv1Desc: "Покупайте напрямую на нашем производстве в Нинбо. Исключите посредников и максимизируйте свою прибыль.",
    whyAdv2Title: "Строгий контроль качества",
    whyAdv2Desc: "Каждая партия проходит тщательные испытания, включая тесты в соляном тумане, проверку на озоностойкость и морозоустойчивость.",
    whyAdv3Title: "OEM и ODM кастомизация",
    whyAdv3Desc: "Поддержка лазерной гравировки логотипов, индивидуального дизайна цвета и разработки упаковки под ваш бренд.",
    whyAdv4Title: "Глобальная доставка и поддержка",
    whyAdv4Desc: "Десятилетия опыта экспорта в Северную Америку, Европу, Юго-Восточную Азию и Южную Америку. Быстрое таможенное оформление.",
    aboutTag: "О компании",
    aboutTitle: "Ningbo Zhenhai Bowang Autoparts Co., Ltd.",
    aboutDesc1: "Расположенный в приморском промышленном центре Нинбо (Китай), завод Lelion Autoparts является современным предприятием по производству автомобильных щеток стеклоочистителя. На площади более 8 000 кв. метров расположены цеха разработки, конструирования, производства и дистрибуции.",
    aboutDesc2: "Завод оснащен автоматизированными сборочными линиями, высокоточными станками для резки каучуковой ленты и передовой испытательной лабораторией. Годовой объем выпуска превышает 12 млн единиц.",
    aboutStat1Title: "15+",
    aboutStat1Desc: "Лет опыта",
    aboutStat2Title: "8 000㎡",
    aboutStat2Desc: "Площадь завода",
    aboutStat3Title: "50+",
    aboutStat3Desc: "Экспортирующих стран",
    aboutPledgeTitle: "Наше обещание качества",
    aboutPledgeQuote: `\"Качество — это основа Lelion. Мы импортируем только высококлассный каучук класса ААА, используем прочную пружинную сталь и проводим 100% контроль качества. Мы гарантируем бесшумную очистку без полос на долгий срок для всех клиентов по всему миру.\"`,
    aboutPledgeAuthor: "— Директор по производству, Lelion Autoparts",
    contactTitle: "Форма запроса",
    contactSubtitle: "Отправьте нам сообщение, и наша команда свяжется с вами в течение 12 часов.",
    contactNameLabel: "Ваше имя *",
    contactEmailLabel: "Email *",
    contactCompanyLabel: "Название компании",
    contactProductLabel: "Интересующий продукт *",
    contactMessageLabel: "Детали / Сообщение *",
    contactWhatsappLabel: "WhatsApp / Телефон",
    contactWhatsappPlaceholder: "+86 188 6788 6795",
    contactQuantityLabel: "Предполагаемое количество",
    contactQuantityPlaceholder: "например, 500 шт. / пробный заказ",    contactNamePlaceholder: "Иван Иванов",
    contactEmailPlaceholder: "ivan@company.com",
    contactCompanyPlaceholder: "Импортер запчастей",
    contactMessagePlaceholder: "Укажите интересующие размеры, количество...",
    contactSuccessMsg: "Спасибо! Ваш запрос успешно отправлен.",
    contactErrorMsg: "Ошибка: ",
    contactSubmitBtn: "Отправить запрос",
    contactSubmittingBtn: "Отправка...",
    contactPageTitle: "Свяжитесь с Lelion",
    contactPageDesc: "Станьте партнером ведущего китайского производителя щеток стеклоочистителя.",
    contactInfoTitle: "Контактная информация",
    contactAddressTitle: "Адрес завода",
    contactPhoneTitle: "WhatsApp / Телефон",
    contactSocialTitle: "Мы в соцсетях",
    faqTitle: "Часто задаваемые вопросы",
    faqSubtitle: "Быстрые ответы на типичные вопросы по B2B закупкам для Lelion.",
    faqQ1: "Каков ваш минимальный объем заказа (MOQ)?",
    faqA1: "Стандартный MOQ составляет 100 шт. на размер для большинства моделей. Пробные заказы обсуждаются.",
    faqQ2: "Предоставляете ли вы бесплатные образцы?",
    faqA2: "Да, образцы бесплатны. Вам нужно оплатить только международную доставку, стоимость которой будет вычтена из вашего первого оптового заказа.",
    faqQ3: "Можете ли вы производить продукцию под моим брендом (OEM)?",
    faqA3: "Да, мы поддерживаем OEM, включая лазерную гравировку логотипа и разработку упаковки под ваш фирменный стиль.",
    faqQ4: "Каковы сроки производства?",
    faqA4: "Сроки составляют 15-25 дней. Мы гарантируем быструю отгрузку из порта Нинбо.",
    faqQ5: "Сертифицирована ли ваша продукция?",
    faqA5: "Да, завод сертифицирован по ISO 9001:2015. Щетки протестированы на 1,5 млн циклов.",
    faqQ6: "Какие условия оплаты вы принимаете?",
    faqA6: "Мы принимаем T/T (30% депозит, 70% против копии B/L), L/C и Western Union для образцов.",
    footerDesc: "Ningbo Zhenhai Bowang Autoparts Co., Ltd. занимается производством высококачественных щеток стеклоочистителя по мировым стандартам и поставляет их по всему миру.",
    footerQuickLinks: "Быстрые ссылки",
    footerContactInfo: "Контакты",
    footerTagline: "Lelion Wiper — Не все дворники понимают сердцебиение стекла",
    footerCta: "Отправить Запрос",
    footerProducts: "Продукты",
    footerInformation: "Информация",
    footerContact: "Контакты",
    footerWhatsapp: "WhatsApp:",
    footerBrandDesc: "LELION — ведущий производитель в отрасли стеклоочистителей, с более чем 15-летним опытом и клиентами в более чем 50 странах.",
    footerCopyright: "© 2026 Ningbo Zhenhai Bowang Autoparts Co., Ltd. Все права защищены.",
    footerUniversal: "Универсальный Стеклоочиститель",
    footerSpecific: "Специализированный Стеклоочиститель",
    footerMultifunction: "Многофункциональный Стеклоочиститель",
    footerHybrid: "Гибридный Стеклоочиститель",
    footerWiperArm: "Рычаг Стеклоочистителя",
    footerRearWiper: "Задняя Щетка",
    footerRearCombo: "Задний Комплект",
    footerFrameWiper: "Каркасная щетка",
    footerFramelessWiper: "Бескаркасная щетка",
    footerColoredWiper: "Цветная щетка",
    footerElectricSpray: "Щетка с электрораспылением",
    footerBusTruck: "Щетка для автобусов и грузовиков",
    footerSnowWiper: "Зимняя щетка",
    footerSpecialWiper: "Специальная щетка",
    footerWiperParts: "Комплектующие щеток",
    footerRearWiperArm: "Задний рычаг",
    footerAddress: "Район Чжэньхай, город Нинбо, провинция Чжэцзян, Китай",
    featuredCategories: "Популярные Категории",
    enterProductCenter: "Перейти в Каталог",
    productsBannerTitle: "Оптовые Решения Стеклоочистителей",
    productsBannerSub: "Ведущий OEM/ODM производитель в Китае, поставляющий качественные компоненты стеклоочистителей для глобальных дистрибьюторов.",
    seriesFilter: "Фильтр Серий",
    allWipers: "Все Стеклоочистители",
    categoryNotFound: "Категория Не Найдена",
    backToProducts: "← Все Товары",
    needCustomSolution: "Нужно Индивидуальное Решение?",
    needCustomSolutionDesc: "Мы предлагаем OEM/ODM услуги с индивидуальным брендингом, упаковкой и рецептурами.",
    contactOurTeam: "Связаться с Нами",
    blogPageTitle: "Блог Индустрии Стеклоочистителей",
    blogPageSub: "Экспертные материалы о производстве стеклоочистителей, стандартах качества и тенденциях отрасли для B2B покупателей.",
    viewProducts: "Смотреть Товары",
    inquiry: "Запрос",
    details: "Детали",
    prev: "← Назад",
    packagingTitle: "OEM и Упаковка под Собственной Маркой",
    packagingDesc: "Гибкие варианты упаковки для розничной, оптовой и послепродажной дистрибуции, включая цветные коробки, блистерные карты, печать логотипов, этикетки со штрих-кодами, многоязычные инструкции и маркировку экспортных коробок.",
    next: "Вперед →",
    categoryBannerSub: "Оптовые щетки стеклоочистителя от ведущего китайского OEM/ODM производителя.",
    categoryNotFoundDesc: "В этой категории нет товаров.",
    catUniversal: "Универсальные Щетки",
    catSpecificFit: "Специальные Щетки",
    catMultifunction: "Многофункциональные Щетки",
    catWiperArm: "Рычаг Стеклоочистителя",
    catRearWiper: "Задняя Щетка",
    catHybrid: "Гибридные Щетки",
    catRearWiperCombo: "Задний Комплект",
    catFrameWiper: "Каркасные щетки",
    catFramelessWiper: "Бескаркасные щетки",
    catColoredWiper: "Цветные щетки",
    catElectricSpray: "Щетки с электрораспылением",
    catBusTruck: "Щетки для автобусов и грузовиков",
    catSnowWiper: "Зимние щетки",
    catSpecialWiper: "Специальные щетки",
    catWiperParts: "Комплектующие щеток",
    catRearWiperArm: "Задние рычаги",
    readMore: "Читать Далее →",
    blogCtaTitle: "Хотите Узнать Больше о Нашей Продукции?",
    blogCtaSub: "Просмотрите наш полный каталог щеток стеклоочистителя или свяжитесь с нашим экспортным отделом для оптовых цен.",
    wiperBladesWord: "Щетки Стеклоочистителя",
    aboutPageTitle: "О Lelion Autoparts | Производитель Щеток OEM/ODM",
    aboutPageDesc: "Сертифицированный ISO 9001 производитель щеток стеклоочистителя. Фабрика 8 000 м², более 200 сотрудников, экспорт в более чем 50 стран с 2011 года.",
    wholesaleStatus: "Статус Опта:",
    wholesaleStatusValue: "Прямо с Фабрики · В Наличии · Готов к Отгрузке",
    minOrderQty: "Минимальный Объем Заказа:",
    productHighlights: "Ключевые Особенности:",
    targetMarkets: "Целевые Рынки:",
    oemServicesTitle: "Доступные Услуги OEM/ODM:",
    oemService1: "Собственная Марка и Лазерная Печать Логотипа",
    oemService2: "Индивидуальная Розничная и Экспортная Упаковка",
    oemService3: "Глобальная Логистика и Таможенная Документация",
    getBulkPrice: "Получить Оптовую Цену →",
    downloadCatalog: "Скачать Каталог",
    techSpecsTitle: "Технические Характеристики —",
    techSpecsSubtitle: "Подробные технические характеристики для",
    whyChooseTitle: "Почему Выбрать",
    whyChooseSubtitle: "Создано для производительности, рассчитано на долговечность, поддержано сертифицированным китайским заводом щеток.",
    faqSectionTitle: "Часто Задаваемые Вопросы —",
    faqSectionSubtitle: "Распространенные запросы B2B о наших",
    moreCatProductsTitle: "Больше",
    moreCatProductsSubtitle: "Ознакомьтесь с нашим полным ассортиментом",
    viewDetails: "Подробнее →",
    relatedInsightsTitle: "Связанные Отраслевые Статьи",
    relatedInsightsSubtitle: "Узнайте больше о качестве щеток, закупках и отраслевых тенденциях в нашем блоге.",
    finalCtaTitle: "Готовы Разместить Оптовый Заказ?",
    finalCtaSub: "Свяжитесь с нашей экспортной командой для получения конкурентных фабричных цен на",
    inquireNow: "Запросить Сейчас",
    downloadFullCatalog: "Скачать Полный Каталог",
    productNotFoundTitle: "Товар Не Найден",
    relatedArticles: "Похожие Статьи",
  popularProducts: "Популярные модели щеток",
    notFoundGeneric: "Не Найдено",
    back: "Назад",
    emailLabel: "ПОЧТА:",
    addressLabel: "Адрес:",
    contactSupportTitle: "Оптовая и OEM Поддержка",
    contactSupportDesc: "Наш экспортный отдел предоставляет индивидуальные технические консультации.",
    contactOfficialEmail: "Официальная Почта",
    contactAvailableOn: "Доступно в WhatsApp и WeChat",
    whatsappMe: "Написать в WhatsApp",
    emailUs: "Написать Нам",
    leadTimeLabel: "Срок поставки",
    leadTimeValue: "15-25 дней",
    trustMoa: "MOQ 100 шт/размер",
    trustLeadTime: "Срок 15 дней",
    trustShipping: "50+ стран",
    trustCert: "ISO 9001 + CE",
    trustSample: "Бесплатные образцы",
    trustSampleNote: "Доставку образцов оплачивает покупатель",
    statsTitle: "Завод в цифрах",
    statsSubtitle: "Проверенные производственные мощности за каждой щёткой Lelion.",
    stat1Value: "15+",
    stat1Label: "Лет опыта",
    stat2Value: "7 млн+",
    stat2Label: "Изделий в год",
    stat3Value: "50+",
    stat3Label: "Стран экспорта",
    stat4Value: "1,5 млн+",
    stat4Label: "Циклов на износ",
    videoQcTitle: "Удалённый видео-контроль",
    videoQcDesc: "Перед отгрузкой доступна удалённая видео-проверка в реальном времени или видеозаписи проверки продукции от вашего менеджера.",
    watchFactoryVideo: "Смотреть видео о заводе",
    reviewsTitle: "Отзывы клиентов",
    reviewsSubtitle: "Реальные отзывы проверенных покупателей на Alibaba.com.",
    reviewsSource: "Проверенные отзывы из нашего магазина на Alibaba.com",
  };
